...
import {listaAvisos} from '../plugins/fantasy_start.js';
...
const cantidadUsuariosRanking = 5;
...
// Aquí explico como se maneja los remitentes Jjjj
  ...
  if (m.isGroup) {
    if (text) { // Si es un grupo y hay texto, se asume que el usuario está ingresando un número de teléfono
      const userArg = text.replace(/[^\d]/g, ''); // Eliminar todos los caracteres que no sean dígitos
  ...
  async function obtenerPersonajeAleatorio() {
    if (usuarioExistente && usuarioExistente[userId] && usuarioExistente[userId].fantasy && usuarioExistente[userId].fantasy.length > 0) {
      const personajes = usuarioExistente[userId].fantasy;
      const personajeAleatorio = personajes[Math.floor(Math.random() * personajes.length)].name;
      return personajeAleatorio;
    } else {
      const personajes = data.infoImg;
      const personajeAleatorio = personajes[Math.floor(Math.random() * personajes.length)].name;
      return personajeAleatorio;
    }
  }
  ...
  let totalLikes = 0; let totalSuperlikes = 0; let totalDislikes = 0;
  ...
  let maxCount = 0; let minCount = Infinity;
  ...
  const usuariosConMasPersonajes = fantasyDB
      .map((entry) => ({
  ...
  const topUsuariosPersonajes = usuariosConMasPersonajes
  ...
  const rankingPersonajes = topUsuariosPersonajes ? topUsuariosPersonajes : '```Todavía no hay usuarios aquí```';

  // Obtener usuarios activos en calificación de personajes
  const usuariosActivos = fantasyDB.map((entry) => ({
    userId: Object.keys(entry)[0],
  ...
  const topUsuariosCalificaciones = usuariosActivos.slice(0, cantidadUsuariosRanking).map((usuario, index) => {
    const positionEmoji = index === 0 ? '🥇 »' : index === 1 ? '🥈 »' : index === 2 ? '🥉 »' : `${index + 1}.`;
    return `*${positionEmoji}* @${usuario.userId.split('@')[0]}\n*✪* Realizó *${usuario.totalCalificaciones}* ${usuario.totalCalificaciones === 1 ? 'calificación' : 'calificaciones'}`;
  }).join('\n\n');
  const rankingCalificaciones = topUsuariosCalificaciones ? topUsuariosCalificaciones : '```Todavía no hay usuarios aquí```';
  ...
  const topUsuariosCaros = [];
  for (const usuario of preciosPersonajes) {
    if (!processedUsers.has(usuario.userId)) {
      const positionEmoji = topUsuariosCaros.length === 0 ? '🥇 »' : topUsuariosCaros.length === 1 ? '🥈 »' : topUsuariosCaros.length === 2 ? '🥉 »' : `${topUsuariosCaros.length + 1}.`;
  ...
  const rankingCaros = topUsuariosCaros.length > 0 ? topUsuariosCaros.join('\n\n') : '```Todavía no hay usuarios aquí```';
  /* preciosPersonajes.sort((a, b) => b.precio - a.precio)
...
let rankingCaros = topUsuariosCaros ? topUsuariosCaros : 'Todavía no hay usuarios aquí'*/
  ...
  const mejoresClasesPorUsuario = {};
  Object.keys(clasesPorUsuario).forEach((userId) => {
    const clasesUsuario = clasesPorUsuario[userId];
    const mejorClase = Object.keys(clasesUsuario).reduce((a, b) => clasesUsuario[a] > clasesUsuario[b] ? a : b);
    mejoresClasesPorUsuario[userId] = mejorClase;
  });
  // Ordenar a los usuarios según la cantidad de personajes en su mejor clase
  const topUsuariosClases = Object.keys(mejoresClasesPorUsuario)
      .filter((userId) => Object.values(clasesPorUsuario[userId]).length > 0)
      .sort((a, b) => {
        const aClass = validClasses.indexOf(mejoresClasesPorUsuario[a]);
        const bClass = validClasses.indexOf(mejoresClasesPorUsuario[b]);
        return bClass - aClass;
      })
      .slice(0, cantidadUsuariosRanking)
      .map((userId, index) => {
        const clase = mejoresClasesPorUsuario[userId];
        const count = clasesPorUsuario[userId][clase];
        const positionEmoji = index === 0 ? '🥇 »' : index === 1 ? '🥈 »' : index === 2 ? '🥉 »' : `${index + 1}.`;
        return `*${positionEmoji}* @${userId.split('@')[0]}\n*✪ ${clase}* » *${count}* personaje${count === 1 ? '' : 's'}`;
      }).join('\n\n');
  const rankingClases = topUsuariosClases ? topUsuariosClases : '```Todavía no hay usuarios aquí```';

  // Usuarios por cantidad de transferencias
  const usuariosTransferencias = fantasyDB
      .map((entry) => {
  ...
  const rankingTransferencias = usuariosTransferencias ? usuariosTransferencias : '```Todavía no hay usuarios aquí```';
  ...
  const contexto2 = user.fantasy_character2 === 0 ? '¡Compra varios personajes!' :
...
'💸 Recompensa magistral 💸';
  ...
  const contexto3 = user.fantasy_character3 === 0 ? '¡Califica a varios personajes!' :
...
'💸 Recompensa magistral 💸';
  ...
  const contexto4 = user.fantasy_character4 === 0 ? '¡Califica a varios personajes!' :
...
'💸 Recompensa magistral 💸';
  ...
  const contexto5 = user.fantasy_character5 === 0 ? '¡Califica a varios personajes!' :
...
*✓ Tipo:* _${contexto5}_
`;
  // let fake = { contextInfo: { externalAdReply: { title: `🌟 FANTASÍA RPG`, body: `😼 RPG de: » ${conn.getName(userId)}`, sourceUrl: accountsgb.getRandom(), thumbnailUrl: 'https://i.pinimg.com/originals/2b/c1/06/2bc10639d4f5cf5685185.jpg' }}}
  const image = [
...
/* await conn.sendFile(m.chat, 'https://i.pinimg.com/originals/77/cd/4b/77cd4b654273b5cde1ce8.jpg', 'fantasy.jpg', mensaje.trim(), fkontak, null, {
contextInfo: {
'forwardingScore': 200,
'isForwarded': false,
externalAdReply: {
...
thumbnailUrl: 'https://i.pinimg.com/originals/2b/c1/06/2bc10639d4f5cf5685185.jpg'
}}})*/
