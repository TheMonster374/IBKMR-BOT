...
const handler = async (m, {conn, usedPrefix, command, groupMetadata, participants, isPrems}) => {
  ...
  try {
    ...
    if (global.db.data.users[m.sender].money < 0) return m.reply(`《💰》${pickRandom(global.robar)} ${money} 🪙 Coins`).catch(global.db.data.users[m.sender].money += money);

    const or = ['text', 'text2', 'text3', 'text4', 'text5'];
    const media = or[Math.floor(Math.random() * 5)];
    ...
    if (media === 'text3') return m.reply(`《💰》${pickRandom(global.robar)}\n\n${diamond} 💎 𝐃𝐈𝐀𝐌𝐀𝐍𝐓𝐄\n${money} 🪙 𝐂𝐎𝐈𝐍𝐒`).catch(global.db.data.users[m.sender].limit += diamond).catch(global.db.data.users[m.sender].money += money);
