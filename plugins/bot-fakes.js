...
────────────────────────`;
  ...
  if (command == 'fakeloc') {
    ...
    const msg = await generateWAMessageFromContent(m.chat, {locationMessage: {degreesLatitude: 0, degreesLongitude: 0, name: 'Github del bot', address: '', url: md, isLive: true, accuracyInMeters: 0, speedInMps: 0, degreesClockwiseFromMagneticNorth: 2, comment: 'Hola', jpegThumbnail: imagen3}}, {quoted: m});

    conn.relayMessage(m.chat, msg.message, {});
  }

  if (command == 'fake2img') {
    conn.sendMessage(m.chat, {image: imagen1, caption: wm, mentions: [...text.matchAll(/@([0-9]{5,16}|0)/g)].map((v) => v[1] + '@s.whatsapp.net'), contextInfo: {'mentionedJid': [...text.matchAll(/@([0-9]{5,16}|0)/g)].map((v) => v[1] + '@s.whatsapp.net'), 'externalAdReply': {'showAdAttribution': true, 'containsAutoReply': true, 'renderLargerThumbnail': true, 'title': wm, 'containsAutoReply': true, 'mediaType': 1, 'thumbnail': imagen2, 'mediaUrl': md, 'sourceUrl': md}}}, {quoted: fproducto});
  }

  if (command == 'sendurl') {
    const str = '*test*';
    await conn.sendUrl(m.chat, str, m, {externalAdReply: {mediaType: 1, renderLargerThumbnail: true, thumbnail: imagen1, thumbnailUrl: imagen1, title: ''}});
  }
