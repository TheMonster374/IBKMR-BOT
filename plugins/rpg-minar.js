const handler = async (m, {conn, isPrems}) => { // lastmiming
  ...
  const pp = 'https://media.istockphoto.com/vectors/basic-rgb-vector-id1315251368?b=1&k=6&m=1315251368&s=170667a&w=0&h=2BgQx5Pu2CewGeq93Qxsyoyw5oT4gioHOOIkHb7PoyY=';

  const string = `${pickRandom([1, 2, 3, 4, 5])}` * 1;
  const stringpremium = `${pickRandom([3, 4, 6, 7, 9, 11])}` * 1;

  const coal = `${pickRandom([4, 5, 8, 10, 12])}` * 1;
  const coalpremium = `${pickRandom([9, 14, 15, 17, 20])}` * 1;

  const emas = `${pickRandom([1, 0, 2, 3, 0, 0, 0])}` * 1;
  const emaspremium = `${pickRandom([2, 4, 5, 1, 1, 7, 8])}` * 1;
  ...
  const xp = `${pickRandom([100, 200, 250, 300, 370, 400, 450, 480, 500, 510, 640, 680, 704, 760, 800, 840, 880, 900, 1000, 1059, 1080, 1100, 1190, 1230, 1380, 1399, 1290, 1300, 1340, 1350, 1590, 1400, 1450, 1700, 1800, 1900, 2000, 0, 0, 10, 1, 99, 999, 1789, 1430])}` * 1;
  const exppremium = `${pickRandom([500, 600, 700, 800, 900, 1000, 1050, 1150, 1200, 1250, 1300, 1350, 1400, 1450, 1500, 1550, 1600, 1650, 1700, 1750, 1800, 1850, 1950, 2000, 2100, 2200, 2300, 2400, 2500, 2600, 2700, 2800, 2900, 3000, 3100, 3200, 3400, 3500, 3600, 3700, 3800, 3850, 3900, 3950, 4000])}` * 1;

  const time = user.lastmiming + 600000; // 10 min
