const handler = async (m, {conn, args}) => {
  ...
  const mention = m.mentionedJid[0] || conn.parseMention(args[0]) || (args[0].replace(/[@.+-]/g, '').replace(' ', '') + '@s.whatsapp.net') || '';
  ...
  if ((user.warn * 1) < 3) {
    ...
    const reason = (args.length > 0 || args[1] ? args.slice(1).join(' ') : '4 kali WARN') || '4 kali WARN';
    user.banned = true;
