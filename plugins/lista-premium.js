const handler = async (m, {conn, isOwner}) => {
  const vip = global.db.data.users[m.sender].premium;
  const prem = Object.entries(global.db.data.users).filter((user) => user[1].premium);
  const caption = `🎟️ 𝙐𝙎𝙐𝘼𝙍𝙄𝙊𝙎 𝙋𝙍𝙀𝙈𝙄𝙐𝙈 : 𝙑𝙄𝙋 𝙐𝙎𝙀𝙍𝙎
*╭•·–––––––––––––––––––·•*
