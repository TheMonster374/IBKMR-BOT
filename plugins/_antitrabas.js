...
export async function before(m, {conn, isAdmin, isBotAdmin, text, participants, usedPrefix}) {
  ...
  const fakemek = {'key': {'participant': '0@s.whatsapp.net', 'remoteJid': '0@s.whatsapp.net'}, 'message': {'groupInviteMessage': {'groupJid': '51995386439-1616969743@g.us', 'inviteCode': 'm', 'groupName': 'P', 'caption': 'Jotchua-Mini', 'jpegThumbnail': null}}};
  if (chat.antiTraba && m.text.length > 4000) { // Cantidad máxima de caracteres aceptados en un mensaje//
    ...
    if (isBotAdmin) {
      ...
      setTimeout(() => {
        conn.sendMessage(m.chat, {text: `Marcar el chat como leido ✓\n${'\n'.repeat(400)}\n• El número : wa.me/${m.sender.split('@')[0]}\n• Alias : ${name}\n⚠️ Acaba de enviar un texto que contiene muchos caracteres que puede ocasionar fallos en los dispositivos`, mentions: [m.sender]}, {quoted: fakemek, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100});
      }, 0);
      setTimeout(() => {
