...
const handler = async (m, {conn, args: [effect], text: txt, usedPrefix, command, name}) => {
  if (!effect) throw '*_como usar el comando_*\n-> _#logo (efecto) (texto)_\n*ejemplo:*\n--> _#logo 3d-deep-sea-metal Jotchua-Mini_\n\n*[❗] cuando diga que hace falta un texto el uso seria:*\n-> _#logo (efecto) (texto1|texto2)_\n*ejemplo:*\n-> _#logo Wolf-Logo-Galaxy Jotchua|Bot_\n\n*<𝑳𝑰𝑺𝑻𝑨 𝑫𝑬 𝑬𝑭𝑬𝑪𝑻𝑶𝑺/>*\n\n° --> #logo ' + effects.map((v) => v.title).join('\n° -> #logo ');
  if (!effects.find((v) => (new RegExp(v.title, 'gi')).test(effect))) throw `*El efecto ${effect} no esta en la lista de efectos*`;
  ...
  const res = await maker(effectoSelect?.url, [...text]).catch((_) => {
    throw '*[❗] Falta el texto al que se realizara el logo*';
  });
  if (typeof res == 'number') throw res == -1 ? `*el efecto ${effect} no esta en la lista de efectos*` : `*el uso correcto del comando es: ${usedPrefix + command} ${effect} ${new Array(res).fill('texto').map((v, i) => v + (i ? i + 1 : '')).join('|')}*`;
  await conn.sendMessage(m.chat, {image: {url: res.image}, caption: `*_aqui tienes tu imagen personalizada_*\n*efecto: ${effect}*`}, {quoted: m});
};
handler.help = ['logos'];
