...
import fs from 'fs';
const handler = async (m, {conn, usedPrefix, command, args, isOwner, isAdmin, isROwner, text}) => {
// try{
  ...
  const aa = toUser + '@s.whatsapp.net';
  const titulo = [
...
lenguajeGB.smsParaAdYOw() + ' ' + `${m.isGroup ? chat.reaction ? '✅' : '❌' : lenguajeGB.smsNoGg()}`,    */
    ...
    lenguajeGB.smsParaAdYOw() + ' ' + `${m.isGroup ? chat.nsfw ? '✅' : '❌' : lenguajeGB.smsNoGg()}`,
...
[
  ...
  const comando = ['welcome',
    ...
    'gconly',
    'pconly'];
  const sections = Object.keys(titulo, nombre, descripción, comando).map((v, index) => ({title: `${titulo[v]}`,
...
╰━━━━━✦ 🤍 ✦━━━━━⬣
> Si no funciona, use .on2/off2
> Inabakumori-Bot                                                   By Monster`, // `
    title: null,
    buttonText: ` ${lenguajeGB.smsConfi1()} ⚙️`,
    sections};
  ...
  switch (type) {
    ...
    case 'nsfw': case 'modocaliente': case 'modehorny':
      if (m.isGroup) {
        if (!(isAdmin || isOwner)) {
          global.dfail('admin', m, conn);
          throw false;
 ...
 */
    ...
    case 'antireadviewonce': case 'modover': case 'modoobservar': case 'modobservar': case 'antiviewonce':
      if (m.isGroup) {
        if (!(isAdmin || isOwner)) {
          global.dfail('admin', m, conn);
          throw false;
