...
const handler = async (m, {text, usedPrefix, command, conn}) => {
  ...
  if (m.quoted && m.quoted.sender && text) {
    ...
    const [userText, characterText] = text.split(/[|,&\/\\]+/).map((v) => v.trim());
    ...
    const isUserNumber = userText.endsWith('@s.whatsapp.net');
    const isCharNumber = characterText.endsWith('@s.whatsapp.net');
    if (isUserNumber) {
      ...
      const hasFiveDigitsUser = (userText.match(/\d/g) || []).length >= 5;
      const hasFiveDigitsChar = (characterText.match(/\d/g) || []).length >= 5;
  ...
  const senderIndex = fantasyDB.findIndex((obj) => obj.hasOwnProperty(userId));
  if (senderIndex == -1) return conn.reply(m.chat, `> *Primero compra un personaje usando:*\n\n\`${usedPrefix}fantasy o ${usedPrefix}fy\``, m);
  const recipientIndex = fantasyDB.findIndex((obj) => obj.hasOwnProperty(user));
  ...
  const senderData = fantasyDB[senderIndex][userId];
  const characterIndex = senderData.fantasy.findIndex((obj) => obj.name == character || obj.id == character);
  ...
  let id_message;
  if (characterIndex != -1) {
    const mensajeConfirmacion = `> *Esto pasará si transfieres "${senderData.fantasy[characterIndex].name}" a @${user.split('@')[0]}*\n
...
> _Si deseas continuar con la transferencia, escriba *"Si"* respondiendo a este mensaje, de lo contrario escriba *"No"*_`;
  ...
  const usuarioExistente = fantasyDB.find((usuario) => Object.keys(usuario)[0] === m.sender);
  handler.before = async (m) => {
    ...
    if (m.quoted && m.quoted.id == id_message && ['si', '👍'].includes(m.text.toLowerCase())) {
      const receiverIndex = recipientIndex;

      for (let i = 0; i < senderData.fantasy.length; i++) {
        const characterData = senderData.fantasy[i];
      ...
      const userReceiverDB = fantasyDB.find((userEntry) => userEntry[user]);
      if (senderCharacter && userInDB && userReceiverDB) {
        ...
        const fytxt = `> *Transferencia completada* ✅\n
